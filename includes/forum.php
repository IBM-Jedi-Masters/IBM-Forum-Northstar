...
class asgarosforum {
    ...
    function movethread() {
        if ($this->is_moderator() && $this->access) {
            $strOUT = '<div class="ibm-col-6-4"><form method="post" action="'.$this->url_movethread.'&amp;move_thread">';
            $strOUT .= sprintf(__('<div class="">Move "<strong>%s</strong>" to new forum:</div>', 'asgaros-forum'), $this->get_name($this->current_thread, $this->table_threads)).'<br />';
            ...
            $strOUT .= '</select><br /><div class="ibm-padding-top-1"><input class="ibm-btn-pri" type="submit" value="'.__('Move', 'asgaros-forum').'" /></div></form></div>';
    ...
    function post_menu($post_id, $author_id, $counter) {
        global $user_ID;
        $o = '<p class="ibm-button-link ibm-btn-row ibm-background-alternate">';

        if ($user_ID && (!$this->get_status('closed') || $this->is_moderator())) {
            $o .= '<a href="'.$this->url_editor_post.'&amp;quote='.$post_id.'" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-quotes-left"></span>'.__('Quote', 'asgaros-forum').'</a>';
        }

        if (($counter > 1 || $this->current_page >= 1) && $this->is_moderator()) {
            $o .= '<a class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50" onclick="return confirm(\''.__('Are you sure you want to remove this?', 'asgaros-forum').'\');" href="'.$this->get_link($this->current_thread, $this->url_thread).'&amp;remove_post&amp;post='.$post_id.'"><span class="icon-bin"></span>'.__('Remove', 'asgaros-forum').'</a>';
        }

        if (($this->is_moderator()) || $user_ID == $author_id) {
            $o .= '<a class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50" href="'.$this->url_editor_edit.$post_id.'&amp;part='.($this->current_page + 1).'"><span class="icon-pencil2"></span>'.__('Edit', 'asgaros-forum').'</a>';
        }

        $o = (!empty($o)) ? $o = '<div class="post-menu">'.$o.'</p></div>' : $o;
    ...
    function forum_menu($location) {
        global $user_ID;
        $menu = '<p class="ibm-button-link ibm-fright ibm-btn-row ibm-background-alternate">';

        if ($user_ID) {
            if ($location == 'forum') {
                $menu .= '<a href="'.$this->url_editor_thread.'" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-file-empty"></span><span>'.__('New Thread', 'asgaros-forum').'</span></a>';
            } else if ($location == 'thread') {
                if (!$this->get_status('closed') || $this->is_moderator()) {
                    $menu .= '<a href="'.$this->url_editor_post.'" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-bubble2"></span><span>'.__('Reply', 'asgaros-forum').'</span></a>';
                }

                if ($this->is_moderator()) {
                    $menu .= '<a href="'.$this->url_movethread.'" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-shuffle"></span><span>'.__('Move Thread', 'asgaros-forum').'</span></a>';
                    $menu .= '<a href="'.$this->url_thread.$this->current_thread.'&amp;delete_thread" onclick="return confirm(\''.__('Are you sure you want to remove this?', 'asgaros-forum').'\');" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-bin"></span><span>'.__('Delete Thread', 'asgaros-forum').'</span></a>';

                    $menu .= '<a href="'.$this->get_link($this->current_thread, $this->url_thread).'&amp;sticky" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-pushpin"></span><span>';
                    $menu .= ($this->get_status('sticky')) ? __('Undo Sticky', 'asgaros-forum') : __('Sticky', 'asgaros-forum');
                    $menu .= '</span></a>';

                    if ($this->get_status('closed')) {
                        $menu .= '<a href="'.$this->get_link($this->current_thread, $this->url_thread).'&amp;closed" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-unlocked"></span><span>'.__('Re-open', 'asgaros-forum').'</span></a>';
                    } else {
                        $menu .= '<a href="'.$this->get_link($this->current_thread, $this->url_thread).'&amp;closed" class="ibm-btn-pri ibm-btn-small ibm-btn-blue-50"><span class="icon-lock"></span><span>'.__('Close', 'asgaros-forum').'</span></a>';
                    }
                }
            }
        }
    ...
    function breadcrumbs() {
        ...
        return '<div class="breadcrumbs ibm-col-6-4">'.$trail.'</div>';
    }
    ...
    function get_thread_image($thread_id, $status) {
        ...
        echo '<span class="icon-files-empty-small'.$unread_status.'"></span>';
    }
